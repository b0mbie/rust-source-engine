...
impl<'str, T> ConVarObject<'str, T> {
	...
	pub fn into_raw(self) -> (T, CConVarExt) {
		let Self { con_var, inner, .. } = self;
		(inner, con_var.data)
	}

	pub const fn name(&self) -> &'str CStr {
		unsafe { CStr::from_ptr(self.con_var.data.base.name) }
	}
	
	pub const fn default(&self) -> &'str CStr {
		unsafe { CStr::from_ptr(self.con_var.data.default_value) }
	}

	pub const fn help(&self) -> Option<&'str CStr> {
		unsafe { opt_c_str_from_ptr(self.con_var.data.base.help_string) }
	}
	...
	T: RawVariable<'str>,
{
	...
	pub const fn unparented(inner: T, params: ConVarParams<'str>) -> Self {
	...
	vtable_methods! {
		...
		fn get_help_text() -> *const c_char {
			...
			T::help(this_pinned);
			this_to_self!(ref this).con_var.data.base.help_string
		}
		...
		fn internal_set_value(value: *const c_char) {
			let value = unsafe { opt_c_str_from_ptr(value) };
			let this = unsafe { this_to_pin_self!(mut this) };
			T::set_c_str(this, value)
		}
		...
		fn clamp_value(mut value: RefMut<c_float>) -> bool {
			...
			let this = unsafe { this_to_pin_self!(mut this) };
			T::clamp_value(this, value)
		}

		fn change_string_value(new_value: *const c_char, old_value: c_float) {
			let new_value = unsafe { opt_c_str_from_ptr(new_value) };
			let this = unsafe { this_to_pin_self!(mut this) };
			T::change_string_value(this, new_value, old_value);
		}
