...
pub fn add_to_register<T: ?Sized + AsStaticRegistrable>(registrable: &'static T) {
	fn add_to_register_impl(registrable: StaticRegistrable) {
		let mut new_first = registrable;
	...
	add_to_register_impl(registrable.as_static_registrable())
}
...
impl<'str, T> AsStaticRegistrable for crate::con::cmd::GenericConCommand<'str, T>
where
	T: crate::con::cmd::GenericCommand<'str>,
{
	fn as_static_registrable(&'static self) -> StaticRegistrable {
		unsafe { StaticRegistrable::new(self.as_registrable()) }
	}
