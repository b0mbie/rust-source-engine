...
use crate::{
	cppdef::{
		ClientIndex, PluginResult, QueryCvarCookie, QueryCvarValueStatus, ServerPluginCallbacksVt, INTERFACEVERSION_ISERVERPLUGINCALLBACKS
	},
	RejectReason,
};
...
pub trait Plugin {
	...
	fn server_activate(&mut self, edicts: &mut [ServerEdict], client_max: c_int) {
		let _ = edicts;
		let _ = client_max;
	}
...
impl<T: Plugin> ToRawInterface for PluginObject<T> {
	...
	T: Plugin,
{
	...
	vtable_methods! {
		...
		fn get_plugin_description() -> *const c_char {
			this_to_self!(mut this).inner.plugin_description().as_ptr()
		}
		...
		fn server_activate(edict_list: *mut Edict, edict_count: c_int, client_max: c_int) {
